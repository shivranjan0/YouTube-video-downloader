...
export class YoutubeService {
    ...
    async handleDownload(url: string, format: string, quality: string, res: Response) {
        ...
        try {
            ...
            if (!res.headersSent) {
                res.status(500).json({ error: 'Failed to fetch video information. YouTube might be blocking the request.' });
            }
        }
    }
    ...
    private async handleVideo(url: string, title: string, quality: string, res: Response) {
        ...
        try {
            ...
            if (!res.headersSent) {
                res.status(500).json({ error: 'Failed to process high-quality video. The requested resolution might not be available or YouTube is blocking the request.' });
            }
            // Cleanup on error
            if (await fs.pathExists(tempFilePath)) {
